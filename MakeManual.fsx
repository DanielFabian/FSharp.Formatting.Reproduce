...
let projectName = "InCube Pricing Solution Manual"
...
type ReferencePage(parent:Document option, srcPath:string, assemblyPath) as this =
    ...
    override this.BuildContent() =
        if Options.Instance.NoReference then
            ...
            MetadataFormat.Generate((assemblyPath:string), outputFolder, [ templatePath ], parameters = projectInfo)
        ...
        for file in (Directory.GetFiles(srcDir)) do
            ...
            | _, ".reference" ->
                let assemblyFolder = Options.Instance.TryAssemblyFolder |> function
                    | Some folder -> folder
                    | None -> Options.Instance.SolutionFolder @@ "results"
...
Options.Instance.DestinationFolder <- "temp/manual"
Options.Instance.SolutionFolder <- "."
printfn "Making ReadKey to attach debugger, if wanted"
